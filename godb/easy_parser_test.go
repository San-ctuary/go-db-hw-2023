...
func TestParseEasy(t *testing.T) {
	var queries []string = []string{
		...
		"select min(age) + max(age) from t", // error
		"select * from t limit 1+2",
		"select t.name, t.age from t join t2 on t.name = t2.name, t2 as t3 where t.age < 50 and t3.age = t.age order by t.age asc, t.name asc", // error
		"select sq(sq(5)) from t", // error
		"select 1, name from t",   // error
		"select age, name from t",
		"select t.name, sum(age) totage from t group by t.name", // error
